...
from src.config import MASTER_LONG_FILENAME, WIDE_SUMMARY_FILENAME
...
if uploaded_files:
    ...
    if master_long_data:
        ...
        with col1:
            st.download_button("Master Long Dataset (CSV)", to_csv(master_long_data), "master_long_dataset.csv", "text/csv")
            st.download_button("Wide Summary (CSV)", to_csv(wide_data), "wide_summary.csv", "text/csv")

        with col2:
            st.download_button("Category Summary (CSV)", to_csv(category_summary), "category_summary.csv", "text/csv")
            st.download_button("Final Scores (CSV)", to_csv(final_scores), "final_score.csv", "text/csv")

        with col3:
            st.download_button("Manual Review Flagged (CSV)", to_csv(all_manual_review), "manual_review.csv", "text/csv")
            st.download_button("Quality Report (CSV)", to_csv(quality_report), "extraction_quality_report.csv", "text/csv")

        if duplicate_conflicts:
            st.warning(f"⚠️ {len(duplicate_conflicts)} Duplicate conflicts detected. Check manual_review.csv.")
            st.download_button("Duplicate Conflicts (CSV)", to_csv(duplicate_conflicts), "duplicate_conflicts.csv", "text/csv")
